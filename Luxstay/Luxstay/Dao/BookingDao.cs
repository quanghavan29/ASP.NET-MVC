        ...
        public List<Booking> findAllBookingByUserId(int user_id)
        {
            try
            {
                ...
                for (int i = 0; i < dataTable.Rows.Count; i++)
                {
                    ...
                    // nếu ngày hiện tại < check_in
                    if (now <= date_check_in)
                    {
                        status = 2; // có thể hủy phòng
                    } // ngày hiện tại nằm trong check in và check out 
                    else if (now <= date_check_out)
                    {
                        status = 1; // đang trong thời gian thuê phòng
                    } // ngày hiện tại > check out
                    else if (now > date_check_out) // hết thời gian thuê
                    {
                        status = 0; // đã check_out
                    }
